...
public class ArrCharOps {
    ...
    public static int indexOf(char[] arr, char ch) {
        //Best practice would be to call indexOf(arr, ch, 0) but this is how I assume you want it
        for (int i = 0; i < arr.length; i++) {
            if(arr[i] == ch) {
                return i;
            }
    ...
    */
    public static char[] concat(char[] arr1, char[] arr2) {
        //Could arr1.copy() but I didn't know if we are allowed so I didn't
        char[] returnArray = new char[arr1.length + arr2.length];
        for (int i = 0; i < arr1.length; i++) {
    ...
    public static char[] subArray(char[] arr, int beginIndex, int endIndex) {
        char[] returnArray;
        //Checks for is the inputs are trying to go out of index and prevents it
        if (endIndex >= arr.length) {
            returnArray = new char[arr.length - beginIndex];
        }
        else {
            returnArray = new char[endIndex - beginIndex];
        }
        for (int i = 0; i < returnArray.length; i++) {
            returnArray[i] = arr[i + beginIndex];
        }
        return returnArray;
    }
    ...
    public static int compareTo(String str1, String str2) {
        //Despite the Documentation saying this is not needed the tests require it
        if(str1.length() == 0 || str2.length() == 0) {
            return -2;
        }

        //Sets len to whichever string is shorter
        ...
        //returns proper value depending on which is bigger
